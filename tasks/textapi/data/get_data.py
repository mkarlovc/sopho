...
def loadNytText():
    ins = open('./data/nyt-text/texts.txt', "rw+")
    line = 1
    while line:
        pos = ins.tell()
        line = ins.readline()
...
def loadNytCluster():
    ins = open('./data/nyt-text/cluster_article_id.txt', "rw+")
    ins.readline()
    line = 1
    while line:
        pos = ins.tell()
...
def idSearch(inId):
    ...
    if lnum != -1:
        lnum += 2
        fo = open('./data/nyt-text/cluster_article_id.txt',"rw+")
        fo.seek(lnum,0)
        line = fo.readline()
...
def fileSearch(inId):
    ...
    if clusterIID.has_key(cid):
        lnum = clusterIID[cid]
        fo = open('./data/nyt-text/texts.txt',"rw+")
        fo.seek(lnum,0)
        line = fo.readline()
        print "line2:", line
